#!/bin/bash
# Backups of the database and the shop files.
#
#   backup.sh            # loop: back up now, then every BACKUP_INTERVAL_HOURS
#   backup.sh now        # one backup
#   backup.sh list       # list backups
#   backup.sh restore <timestamp>   # restore DB and shop files from a backup
...
# (the whole shop directory except caches),
...
backup() {
    ...
    tar -C "${APP_DIR}" --exclude=./var/cache --exclude=./var/logs -czf "${tmp}-files.tar.gz" .
...
restore() {
    ...
    echo "==> Restoring shop files from ${files}"
